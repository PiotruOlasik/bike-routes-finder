...
def save_best_route(best_result: Dict, start_lon: float, start_lat: float, all_results: List[Dict]):
    ...
    bike_type = "szosowy"   # docelowo: pobrane od użytkownika
    bike_surface_rules = {
    ...
    rideability, surfaces, not_allowed = evaluate_bike_surface(
    best_result['route_nodes'], G, bike_type, bike_surface_rules
      )
...
def evaluate_bike_surface(route_nodes, G, bike_type, bike_surface_rules):
    ...
    allowed = set(bike_surface_rules[bike_type])

    not_allowed = [s for s in surfaces_on_route if s not in allowed and s != "unknown"]

    # --- WARUNKI ---
    if surfaces_on_route == {"unknown"}:
        rideability = "WARNING_UNKNOWN"     # przypadek 2
    elif not_allowed:
        rideability = "NOT_ALLOWED"         # przypadek 3
    else:
        rideability = "OK"                  # przypadek 1

    return rideability, list(surfaces_on_route), not_allowed
